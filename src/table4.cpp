...
void run(const std::vector<std::string>& geoms, std::string part_dir, int k,
         int iters, int processors, tomo::util::report& table, bool trivial, bool bisected) {
    ...
    env.spawn(processors, [&](auto& world) {
        ...
        for (auto geom_file : geoms) {
            ...
            auto tree_file = part_dir + name + ".bsp";
            ...
            if (trivial) {
                count_messages(
                    world, block_partitioning, global_volume,
                    (tomo::geometry::trajectory<3_D, T>&)global_geometry, table,
                    name, "trivial", iters);
            }
            if (bisected) {
                count_messages(
                    world, *tree_partitioning, global_volume,
                    (tomo::geometry::trajectory<3_D, T>&)global_geometry, table,
                    name, "bisected", iters);
            }
        }
...
void usage(std::string program_name) {
    std::cout << "Usage: " << program_name
              << " --in GEOMS --part PART_DIR -p PROCS [--trivial] [--bisected]\n";
}

int main(int argc, char* argv[]) {
    auto opts = options{argc, argv};

    if (!opts.required_arguments({"--in", "--part", "-p"})) {
        usage(argv[0]);
        return -1;
    }
    ...
    run(opts.args("--in"), opts.arg("--part"), opts.arg_as_or<int>("-k", -1),
        opts.arg_as_or<int>("-i", 1), opts.arg_as<int>("-p"), table, opts.passed("--trivial"),
        opts.passed("--bisected"));
